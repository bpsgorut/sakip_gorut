...
@section('content')
    ...
    <div class="container mx-auto py-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Reward & Punishment</h1>
                <p class="text-sm text-gray-500 mt-1">Pengelolaan sistem reward dan punishment pegawai untuk setiap tahun berjalan
                </p>
            </div>
                ...
                <table class="w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider" style="width: 15%;">Status</th>
                        ...
                        @forelse($daftar_kegiatan as $kegiatan)
                            <tr class="hover:bg-gray-50 transition-colors"
                                ...
                                <td class="px-6 py-4 text-center">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $kegiatan->kelengkapan ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                ...
                                <td class="px-6 py-4 text-center">
                                    <div class="text-xs text-gray-500">{{ $kegiatan->keterangan }}</div>
                                </td>
                                <td class="px-6 py-4 text-center">
                                    <a href="{{ route('reward.punishment.detail', ['id' => $kegiatan->id]) }}"
                                        class="inline-flex items-center justify-center p-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors"
                                        title="Lihat Detail Reward & Punishment">
                            ...
                            <tr>
                                <td colspan="5" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <h3 class="text-lg font-semibold text-gray-900 mb-2">Belum ada Reward & Punishment</h3>
                                        <p class="text-gray-500 mb-4 max-w-md text-center">Mulai dengan menambahkan sistem reward dan punishment untuk tahun {{ date('Y') }}</p>
                                        <button id="btnTambahKegiatanFirst" class="inline-flex items-center px-4 py-2 bg-red-600 text-white text-sm font-medium rounded-lg hover:bg-red-700 transition-colors">
            ...
            <div class="p-5">
                <div class="relative border-b pb-3 mb-4">
                    <div class="flex justify-center">
                        <h3 class="text-lg font-bold text-red-600">Tambah Reward Punishment</h3>
                    </div>
                ...
                <form action="{{ route('kegiatan.store') }}" method="POST">
                    @csrf
                    <!-- HIDDEN INPUTS -->
                    <input type="hidden" name="jenis_kegiatan" id="jenis_kegiatan" value="reward_punishment">
                    <input type="hidden" name="renstra_id" id="renstra_id" value="{{ $activeRenstra->id ?? '' }}">

                    <!-- SUB KOMPONEN ID -->
                    @php
                        $rewardPunishmentId = null;
                        if (isset($rewardPunishmentSubKomponen) && $rewardPunishmentSubKomponen) {
                            $rewardPunishmentId = $rewardPunishmentSubKomponen->id;
                        } else {
                            $found = $subKomponenList->first(function ($item) {
                                return stripos($item->sub_komponen, 'Reward Punishment') !== false;
                            });
                            if ($found) {
                                $rewardPunishmentId = $found->id;
                            }
                        }
                    @endphp

                    <input type="hidden" name="sub_komponen_id" value="{{ $rewardPunishmentId }}">

                    <!-- Nama Kegiatan (Read Only) -->
                    <div class="mb-4">
                        <label for="nama_kegiatan" class="block text-sm font-bold mb-1">Nama Kegiatan</label>
                        <input type="text" id="nama_kegiatan" name="nama_kegiatan" value="Reward Punishment"
