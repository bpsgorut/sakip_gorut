...
@section('content')
    ...
    <div class="container mx-auto py-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Manajemen PK</h1>
                <p class="text-sm text-gray-500 mt-1">Pengelolaan dokumen perjanjian kinerja untuk setiap tahun berjalan
                </p>
            </div>
            @if($isSuperAdmin)
                ...
                <table class="w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            <th class="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider"
                                style="width: 45%;">Kegiatan</th>
                            <th class="px-6 py-3 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider"
                                style="width: 10%;">Status</th>
                            <th class="px-6 py-3 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider"
                                style="width: 15%;">Kelengkapan</th>
                            @if($isSuperAdmin)
                            <th class="px-6 py-3 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider"
                                style="width: 15%;">Input Target</th>
                            @endif
                                <th class="px-6 py-3 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider"
                                style="width: 10%;">Detail</th>
                            @if($isSuperAdmin)
                            <th class="px-6 py-3 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider"
                                style="width: 8%;">Aksi</th>
                            @endif
                        </tr>
                    </thead>
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($daftar_kegiatan as $kegiatan)
                            @php
                                // Hitung progress berdasarkan kelengkapan
                                $totalProgress = $kegiatan->kelengkapan ? 100 : 0;
                            @endphp
                            <tr class="hover:bg-gray-50 transition-colors" data-year="{{ $kegiatan->tahun_berjalan }}"
                                ...
                                <td class="px-6 py-4 text-center">
                                    @php
                                        // Cek apakah target PK sudah ada - perbaikan query
                                        $hasTargetPk = \App\Models\Target_Pk::where(
                                            'kegiatan_id',
                                            $kegiatan->id,
                                        )->exists();

                                        // Cek apakah masih dalam rentang waktu PK (dari tanggal mulai sampai tanggal berakhir kegiatan)
                                        $currentDate = now();
                                        $startDate = \Carbon\Carbon::parse($kegiatan->tanggal_mulai);
                                        $endDate = \Carbon\Carbon::parse($kegiatan->tanggal_berakhir);
                                        $isWithinTimeLimit =
                                            $currentDate->between($startDate, $endDate) || $currentDate->lt($endDate);

                                        // Tentukan teks dan warna button berdasarkan 3 fase
                                        if (!$hasTargetPk) {
                                            // Fase 1: Belum input target
                                            $buttonText = 'Input Target';
                                            $buttonColor = 'bg-green-600 hover:bg-green-700';
                                            $icon = 'M12 6v6m0 0v6m0-6h6m-6 0H6';
                                        } elseif ($isWithinTimeLimit) {
                                            // Fase 2: Sudah input target dan masih dalam rentang waktu
                                            $buttonText = 'Edit Target';
                                            $buttonColor = 'bg-orange-600 hover:bg-orange-700';
                                            $icon =
                                                'M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.536L16.732 3.732z';
                                        } else {
                                            // Fase 3: Waktu sudah lewat
                                            $buttonText = 'Lihat Target';
                                            $buttonColor = 'bg-blue-600 hover:bg-blue-700';
                                            $icon =
                                                'M15 12a3 3 0 11-6 0 3 3 0 016 0z M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z';
                                        }
                                    @endphp
                                    <a href="{{ route('form.target.pk', ['id' => $kegiatan->id]) }}"
                                        class="inline-flex items-center justify-center px-3 py-2 {{ $buttonColor }} text-white rounded-lg transition-colors text-xs font-medium"
                                        title="{{ $buttonText }}">
                        ...
                        @empty
                            <tr>
                                <td colspan="{{ $isSuperAdmin ? '6' : '5' }}" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <h3 class="text-lg font-semibold text-gray-900 mb-2">Belum ada kegiatan PK</h3>
                        <p class="text-gray-500 mb-4 max-w-md text-center">Tidak ada kegiatan Perjanjian Kinerja untuk tahun {{ date('Y') }}</p>
                        @if($isSuperAdmin)
            ...
            <div class="p-5">
                <div class="relative border-b pb-3 mb-4">
                    <div class="flex justify-center">
                        <h3 class="text-lg font-bold text-red-600">Tambah Kegiatan PK</h3>
                    </div>
                ...
                <form action="{{ route('kegiatan.store') }}" method="POST">
                    @csrf
                    <!-- HIDDEN INPUTS -->
                    <input type="hidden" name="jenis_kegiatan" id="jenis_kegiatan" value="pk">
                    <input type="hidden" name="renstra_id" id="renstra_id" value="{{ $activeRenstra->id ?? '' }}">

                    <!-- SUB KOMPONEN ID -->
                    @php
                        // Cari sub komponen Manajemen PK dengan berbagai cara
                        $manajemenPKId = null;

                        if (isset($manajemenPKSubKomponen) && $manajemenPKSubKomponen) {
                            $manajemenPKId = $manajemenPKSubKomponen->id;
                        } else {
                            ...
                            if ($found) {
                                $manajemenPKId = $found->id;
                            }
                        }
                    @endphp

                    <input type="hidden" name="sub_komponen_id" value="{{ $manajemenPKId }}">

                    <!-- Nama Kegiatan (Read Only) -->
                    <div class="mb-4">
                        <label for="nama_kegiatan" class="block text-sm font-bold mb-1">Nama Kegiatan</label>
                        <input type="text" id="nama_kegiatan" name="nama_kegiatan" value="Perjanjian Kinerja"
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            tabButtons.forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    localStorage.setItem('activePkTab', this.getAttribute('data-tab'));

                    // Remove active class from all tabs
                    tabButtons.forEach(btn => {
                        btn.classList.remove('text-red-600', 'border-b-2',
                            'border-red-600');
                    ...
                    // Show corresponding tab content with fade in
            ...
            const savedTab = localStorage.getItem('activePkTab');
            if (savedTab) {
